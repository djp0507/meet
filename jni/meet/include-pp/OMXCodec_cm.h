...
#ifndef OMX_CODEC_CM_H_

#define OMX_CODEC_CM_H_
...
struct CodecProfileLevel_cm;

struct OMXCodec_cm : public MediaSource,
    ...
    static sp<MediaSource> createCodec(
            const sp<IOMX> &omx,
            const sp<MetaData> &meta, bool createEncoder,
            const sp<MediaSource> &source,
            const char *matchComponentName = NULL,
...
private:
    ...
    OMXCodec_cm(const sp<IOMX> &omx, IOMX::node_id node, uint32_t quirks,
...
protected:
    ...
    OMXCodec_cm(const OMXCodec_cm &);
    OMXCodec_cm &operator=(const OMXCodec_cm &);
...
struct CodecCapabilities_cm {
    String8 mComponentName;
    Vector<CodecProfileLevel_cm> mProfileLevels;
};
