...
#ifndef ANDROID_AUDIOSYSTEM_CM_H_
#define ANDROID_AUDIOSYSTEM_CM_H_
...
public:
    ...
    // Audio sub formats (see AudioSystem_cm::audio_format).
    enum pcm_sub_format {
    ...
    // set audio mode in audio hardware (see AudioSystem_cm::audio_mode)
    ...
    // Some keys are reserved for standard parameters (See AudioParameter_cm class).
...
class AudioParameter_cm {

public:
    AudioParameter_cm() {}
    AudioParameter_cm(const String8& keyValuePairs);
    virtual ~AudioParameter_cm();

    // reserved parameter keys for changing standard parameters with setParameters() function.
    // Using these keys is mandatory for AudioFlinger to properly monitor audio output/input
    // configuration changes and act accordingly.
    //  keyRouting: to change audio routing, value is an int in AudioSystem_cm::audio_devices
    //  keySamplingRate: to change sampling rate routing, value is an int
    //  keyFormat: to change audio format, value is an int in AudioSystem_cm::audio_format
    //  keyChannels: to change audio channel configuration, value is an int in AudioSystem_cm::audio_channels
...
#endif  /*ANDROID_AUDIOSYSTEM_CM_H_*/
