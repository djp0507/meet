...
#include "include-pp/sf/include/AACDecoder.h"
#include "include-pp/sf/include/AVCDecoder.h"
...
#include "include-pp/binder/ProcessState.h"
...
#include "include-pp/media/IMediaPlayerService.h"
...
//#include "media/libstagefright/include/ThreadedSource.h"
...
//    { MEDIA_MIMETYPE_AUDIO_AMR_WB, "OMX.PV.amrdec" },
    ...
    { MEDIA_MIMETYPE_AUDIO_AAC, "OMX.TI.AAC.decode" },
    { MEDIA_MIMETYPE_AUDIO_AAC, "OMX.SEC.aacdec" },
    { MEDIA_MIMETYPE_AUDIO_AAC, "AACDecoder" },
//    { MEDIA_MIMETYPE_AUDIO_AAC, "OMX.PV.aacdec" },
    ...
    { MEDIA_MIMETYPE_VIDEO_MPEG4, "OMX.MARVELL.VIDEO.CODADX8DECODER"},
    { MEDIA_MIMETYPE_VIDEO_MPEG4, "M4vH263Decoder" },
...
//    { MEDIA_MIMETYPE_VIDEO_H263, "OMX.PV.h263dec" },
    ...
    { MEDIA_MIMETYPE_VIDEO_AVC, "OMX.SEC.avcdec" },
//    { MEDIA_MIMETYPE_VIDEO_AVC, "OMX.MARVELL.VIDEO.CODADX8DECODER"}, 
...
struct OMXCodecObserver : public BnOMXObserver {
    ...
    //void setCodec(const sp<OMXCodec_cm> &target) {
    void setCodec(const sp<OMXCodec_cm> &target) {
        mTarget = target;
    }
    ...
    virtual void onMessage(const omx_message &msg) {
        sp<OMXCodec_cm> codec = mTarget.promote();
...
uint32_t OMXCodec_cm::getComponentQuirks(const char *componentName,bool isEncoder, uint32_t flags) {
#else
uint32_t OMXCodec_cm::getComponentQuirks(const char *componentName, bool isEncoder) {
#endif
    uint32_t quirks = 0;
    ...
    if (!strcmp(componentName, "OMX.TI.Video.Decoder") ||
        ...
        if( flags & kPreferThumbnailMode) {
                quirks |= OMXCodec_cm::kRequiresAllocateBufferOnOutputPorts;
        }
    }
#ifdef TARGET_OMAP4
        ...
        if(flags & kPreferThumbnailMode)
        {
            quirks |= OMXCodec_cm::kThumbnailMode;
        }

        if(flags & kPreferInterlacedOutputContent) {
                quirks |= OMXCodec_cm::kInterlacedOutputContent;
        }
...
// static
void OMXCodec_cm::findMatchingCodecs(
        const char *mime,
...
// static
sp<MediaSource> OMXCodec_cm::Create(
        const sp<IOMX> &omx,
...
*/		
        if(flags == kPreferSoftwareCodecs)
        {
	        sp<MediaSource> softwareCodec = InstantiateSoftwareCodec(componentName, source);

	        if (softwareCodec != NULL) {
	            LOGD("Successfully allocated software codec '%s'", componentName);
        ...
        LOGD("Attempting to allocate OMX node '%s'", componentName);
#if defined(OMAP_ENHANCEMENT) || defined(OMAP_COMPAT)
        ...
        if (err == OK) {
            LOGD("Successfully allocated OMX node '%s'", componentName);

            sp<OMXCodec_cm> codec = new OMXCodec_cm(
                    omx, node, quirks,
		...
		{
            LOGE("Failed allocated OMX node '%s'", componentName);
		}
    }
...
status_t OMXCodec_cm::configureCodec(const sp<MetaData> &meta, uint32_t flags) {
#if !(defined(OMAP_ENHANCEMENT) && defined (TARGET_OMAP4))
    ...
    if (!strcasecmp(MEDIA_MIMETYPE_AUDIO_AAC, mMIME)) {
        ...
        if (!strcmp(mComponentName, "OMX.TI.AAC.decode")) {
            ...
            CODEC_LOGV("OMXCodec_cm::configureCodec() TI AAC - Configure Component to FrameMode");
            ...
            if (err != OK) {
                CODEC_LOGV("OMXCodec_cm::configureCodec() TI AAC - Problem getting ExtensionIndex - Use SteamMode");
            }
            else {
                OMX_U16 framemode = 1;
                ...
                if (err != OK) {
                    CODEC_LOGV("OMXCodec_cm::configureCodec() TI AAC - Problem configuring FrameMode - Use SteamMode");
                }
            }
        }
#endif
...
void OMXCodec_cm::setMinBufferSize(OMX_U32 portIndex, OMX_U32 size) {
...
status_t OMXCodec_cm::setVideoPortFormatType(
        OMX_U32 portIndex,
...
status_t OMXCodec_cm::findTargetColorFormat(
...
status_t OMXCodec_cm::isColorFormatSupported(
...
void OMXCodec_cm::setVideoInputFormat(
...
status_t OMXCodec_cm::setupErrorCorrectionParameters() {
...
status_t OMXCodec_cm::setupBitRate(int32_t bitRate) {
...
#if defined (TARGET_OMAP4) && defined (OMAP_ENHANCEMENT)
status_t OMXCodec_cm::setupEncoderPresetParams(int32_t isS3DEnabled) {
...
status_t OMXCodec_cm::setupEncoderFrameDataContentParams(const sp<MetaData>& meta) {
...
status_t OMXCodec_cm::getVideoProfileLevel(
        const sp<MetaData>& meta,
        const CodecProfileLevel_cm& defaultProfileLevel,
        CodecProfileLevel_cm &profileLevel) {
...
status_t OMXCodec_cm::setupH263EncoderParameters(const sp<MetaData>& meta) {
    ...
    CodecProfileLevel_cm defaultProfileLevel, profileLevel;
...
status_t OMXCodec_cm::setupMPEG4EncoderParameters(const sp<MetaData>& meta) {
    int32_t iFramesInterval, frameRate, bitRate;
    bool success = meta->findInt32(kKeyBitRate, &bitRate);
    success = success && meta->findInt32(kKeySampleRate, &frameRate);
    success = success && meta->findInt32(kKeyIFramesInterval, &iFramesInterval);
...
status_t OMXCodec_cm::setupAVCEncoderParameters(const sp<MetaData>& meta) {
    int32_t iFramesInterval, frameRate, bitRate;
...
status_t OMXCodec_cm::setVideoOutputFormat(
...
#if 1
    {
        ...
        static const int OMX_COLOR_Format_meizu_m9 = 0x7F000001;
/*
...
*/		
		...
		LOGI("color %p", color);
		LOGI("format.eColorFormat %p", format.eColorFormat);
        if(format.eColorFormat == OMX_COLOR_FormatYUV420Planar
               ...
               || format.eColorFormat == QOMX_COLOR_FormatYVU420PackedSemiPlanar32m4ka
               || format.eColorFormat == QOMX_COLOR_FormatYUV420PackedSemiPlanar64x32Tile2m8ka
               || format.eColorFormat == OMX_COLOR_Format_meizu_m9)
    	{
    	    //do nothing.
    	    LOGD("do nothing");
    	}
		...
		{
		    LOGE("format.eColorFormat: %p", format.eColorFormat);
		    LOGE("unsupported format.eColorFormat: %x", format.eColorFormat);
			return ERROR_UNSUPPORTED;
		}
    ...
    if(!strcmp(mComponentName, "OMX.TI.DUCATI1.VIDEO.DECODER")) {
		...
		if(mQuirks & OMXCodec_cm::kThumbnailMode){
    ...
    video_def->eColorFormat = color;
   // video_def->eColorFormat = OMX_COLOR_FormatUnused;

   	LOGI("video_def->eColorFormat %p", video_def->eColorFormat);
   	LOGI("video_def->nFrameWidth %d", video_def->nFrameWidth);
   	LOGI("video_def->nFrameHeight %d", video_def->nFrameHeight);
...
// static
void OMXCodec_cm::setComponentRole(
...
status_t OMXCodec_cm::init() {
    // mLock is held.
	LOGI("OMXCodec_cm::init() start!!!");
	...
	LOGI("OMXCodec_cm::init() end!!!");
	LOGI("mState %d", mState);
    return mState == ERROR ? UNKNOWN_ERROR : OK;
}

// static
bool OMXCodec_cm::isIntermediateState(State state) {
...
status_t OMXCodec_cm::allocateBuffers() {
...
status_t OMXCodec_cm::allocateBuffersOnPort(OMX_U32 portIndex) {
    OMX_PARAM_PORTDEFINITIONTYPE def;
    InitOMXParams(&def);
    def.nPortIndex = portIndex;
    ...
    size_t totalSize = def.nBufferCountActual * ((def.nBufferSize + 31) & (~31));
            ...
            if( !strcmp(mComponentName, "OMX.TI.DUCATI1.VIDEO.DECODER") &&
                (mQuirks & OMXCodec_cm::kThumbnailMode) &&
    ...
    if ((!strcmp(mComponentName, "OMX.TI.Video.Decoder") ||
        ...
        mDealer[portIndex] = new MemoryDealer(totalSize, "OMXCodec_cm");
    }
...
#endif
    LOGD("def.nBufferCountActual:%d", def.nBufferCountActual);
...
void OMXCodec_cm::on_message(const omx_message &msg) {
	LOGD("on_message");
    switch (msg.type) {
        case omx_message::EVENT:
        {
            LOGD("%s, omx_message::EVENT", mMIME);
            onEvent(
        ...
        {
            LOGD("%s, omx_message::EMPTY_BUFFER_DONE", mMIME);
            ...
            if (!(*buffers)[i].mOwnedByComponent) {
                LOGD("We already own input buffer %p, yet received "
            ...
            if (mPortStatus[kPortIndexInput] == DISABLING) {
                ...
                CODEC_LOGE("Port is disabled, freeing buffer %p", buffer);
				...
				LOGD("%s, drainning InputBuffer at %d/%d", mMIME, i, buffers->size());
        ...
        case omx_message::FILL_BUFFER_DONE:
        {
			...
			int64_t timestamp = msg.u.extended_buffer_data.timestamp;
	
            LOGD("FILL_BUFFER_DONE %s (buffer: %p, size: %ld, flags: 0x%08lx), timestamp: %lld",
	             mMIME,
                 buffer,
            ...
            if (!info->mOwnedByComponent) {
                LOGD("We already own output buffer %p, yet received "
                     "a FILL_BUFFER_DONE.", buffer);
            }

            info->mOwnedByComponent = false;
			LOGD("mPortStatus[kPortIndexOutput]:%d", mPortStatus[kPortIndexOutput]);
            if (mPortStatus[kPortIndexOutput] == DISABLING) {
                CODEC_LOGI("Port is disabled, freeing buffer %p", buffer);
                ...
                buffer->meta_data()->setInt64(
                        kKeyTime, timestamp);//msg.u.extended_buffer_data.timestamp);
                ...
                if (mTargetTimeUs >= 0) {
                    CHECK(timestamp <= mTargetTimeUs);

                    if (timestamp < mTargetTimeUs) {
                        CODEC_LOGV(
                                "skipping output buffer at timestamp %lld us",
                                timestamp);

						LOGI("%s, filling outputbuffer at %d/%d", mMIME, i, buffers->size());
				...
				LOGD("mFilledBuffers.push_back one buffer, current size: %d", mFilledBuffers.size());
                mBufferFilled.signal();
            } 
			else if ((mQuirks & kRequiresFlushBeforeShutdown) && mState == EXECUTING_TO_IDLE) 
			{
                    ...
                    && countBuffersWeOwn(mPortBuffers[kPortIndexOutput]) == mPortBuffers[kPortIndexOutput].size()) 
                {
                    ...
                    LOGD("own %d/%d input and %d/%d output", countBuffersWeOwn(mPortBuffers[kPortIndexInput]), mPortBuffers[kPortIndexInput].size(),
                        countBuffersWeOwn(mPortBuffers[kPortIndexOutput]), mPortBuffers[kPortIndexOutput].size());
                }
            }

			//LOGD("did not satisfy any condition, break");
            break;
        }
...
void OMXCodec_cm::registerBuffers(const sp<IMemoryHeap> &mem) {
    mPmemInfo = mem;
}
void OMXCodec_cm::onEvent(OMX_EVENTTYPE event, OMX_U32 data1, OMX_U32 data2) {
    switch (event) {
        case OMX_EventCmdComplete:
        {
            LOGD("%s, OMX_EventCmdComplete", mMIME);
        ...
        {
            LOGD("%s, OMX_EventPortSettingsChanged", mMIME);
...
void OMXCodec_cm::onCmdComplete(OMX_COMMANDTYPE cmd, OMX_U32 data) {
    switch (cmd) {
        case OMX_CommandStateSet:
        {
            LOGD("%s, StateSet", mMIME);
        ...
        case OMX_CommandPortDisable:
        {
            ...
            LOGD("%s, PORT_DISABLED(%ld)", mMIME, portIndex);
        ...
        {
            OMX_U32 portIndex = data;
            CODEC_LOGI("%s, PORT_ENABLED(%ld)", mMIME, portIndex);
        ...
        case OMX_CommandFlush:
        {
            ...
            CODEC_LOGI("%s, FLUSH_DONE(%ld)", mMIME, portIndex);
        ...
        default:
        {
            CODEC_LOGI("%s, CMD_COMPLETE(%d, %ld)", mMIME, cmd, data);
            break;
        }
    }
}

void OMXCodec_cm::onStateChange(OMX_STATETYPE newState) {
    ...
    switch (newState) {
        ...
        case OMX_StateExecuting:
        {
            ...
            // call to OMXCodec_cm::read as mInitialBufferSubmit is true at
...
// static
size_t OMXCodec_cm::countBuffersWeOwn(const Vector<BufferInfo> &buffers) {
    size_t n = 0;
...
status_t OMXCodec_cm::freeBuffer(OMX_U32 portIndex, BufferInfo *info) {
...
status_t OMXCodec_cm::freeBuffersOnPort(
...
void OMXCodec_cm::onPortSettingsChanged(OMX_U32 portIndex) {
    CODEC_LOGI("PORT_SETTINGS_CHANGED(%ld)", portIndex);
...
bool OMXCodec_cm::flushPortAsync(OMX_U32 portIndex) {
    CHECK(mState == EXECUTING || mState == RECONFIGURING
...
void OMXCodec_cm::disablePortAsync(OMX_U32 portIndex) {
	LOGD("%s, disablePortAsync", mMIME);
    CHECK(mState == EXECUTING || mState == RECONFIGURING);
    CHECK_EQ(mPortStatus[portIndex], ENABLED);
...
void OMXCodec_cm::enablePortAsync(OMX_U32 portIndex) {
    CHECK(mState == EXECUTING || mState == RECONFIGURING);
...
void OMXCodec_cm::fillOutputBuffers() {
    CHECK_EQ(mState, EXECUTING);
    ...
    Vector<BufferInfo> *buffers = &mPortBuffers[kPortIndexOutput];
    for (size_t i = 0; i < buffers->size(); ++i) {
		LOGD("%s, filling outputbuffer at %d/%d", mMIME, i, buffers->size());
        fillOutputBuffer(&buffers->editItemAt(i));
    }
}

void OMXCodec_cm::drainInputBuffers() {
    CHECK(mState == EXECUTING || mState == RECONFIGURING);
...
void OMXCodec_cm::drainInputBuffer(BufferInfo *info) {
    CHECK_EQ(info->mOwnedByComponent, false);
    ...
    if (mCodecSpecificDataIndex < mCodecSpecificData.size()) {
        ...
        LOGD("calling emptyBuffer with codec specific data");
...
void OMXCodec_cm::fillOutputBuffer(BufferInfo *info) {
    CHECK_EQ(info->mOwnedByComponent, false);
...
void OMXCodec_cm::drainInputBuffer(IOMX::buffer_id buffer) {
    Vector<BufferInfo> *buffers = &mPortBuffers[kPortIndexInput];
    for (size_t i = 0; i < buffers->size(); ++i) {
...
void OMXCodec_cm::fillOutputBuffer(IOMX::buffer_id buffer) {
    Vector<BufferInfo> *buffers = &mPortBuffers[kPortIndexOutput];
    for (size_t i = 0; i < buffers->size(); ++i) {
        if ((*buffers)[i].mBuffer == buffer) {
#if defined(TARGET_OMAP4) && defined(OMAP_ENHANCEMENT)
...
void OMXCodec_cm::setState(State newState) {
    mState = newState;
...
void OMXCodec_cm::setRawAudioFormat(
...
void OMXCodec_cm::setAMRFormat(bool isWAMR, int32_t bitRate) {
...
void OMXCodec_cm::setAACFormat(int32_t numChannels, int32_t sampleRate, int32_t bitRate) {
    if (mIsEncoder) {
        //////////////// input port ////////////////////
        setRawAudioFormat(kPortIndexInput, sampleRate, numChannels);
...
void OMXCodec_cm::setImageOutputFormat(
...
void OMXCodec_cm::setJPEGInputFormat(
...
void OMXCodec_cm::addCodecSpecificData(const void *data, size_t size) {
	...
	LOGV("Push codec specific data with size:%d", size);
}

void OMXCodec_cm::clearCodecSpecificData() {
...
status_t OMXCodec_cm::start(MetaData *meta) {
	LOGI("OMXCodec_cm::start() start!!!");
	...
	LOGI("OMXCodec_cm::start() end!!!");
    return init();
}

status_t OMXCodec_cm::stop() {
...
sp<MetaData> OMXCodec_cm::getFormat() {
    Mutex::Autolock autoLock(mLock);
...
#ifdef OMAP_ENHANCEMENT
void OMXCodec_cm::setBuffers(Vector< sp<IMemory> > mBufferAddresses, bool portReconfig){
    ...
    if(!portReconfig){
        ...
        LOGD("Updating video_def->nStride with new width %d", (int) video_def->nStride);
...
status_t OMXCodec_cm::read(
    ...
    *buffer = NULL;
    ...
    if (seeking) {
        CODEC_LOGI("seeking to %lld us ", seekTimeUs);
...
void OMXCodec_cm::signalBufferReturned(MediaBuffer *buffer) {
    Mutex::Autolock autoLock(mLock);

    Vector<BufferInfo> *buffers = &mPortBuffers[kPortIndexOutput];
	LOGD("signalBufferReturned, output buffers size:%d", buffers->size());
    for (size_t i = 0; i < buffers->size(); ++i) {
        BufferInfo *info = &buffers->editItemAt(i);
...
void OMXCodec_cm::dumpPortStatus(OMX_U32 portIndex) {
    OMX_PARAM_PORTDEFINITIONTYPE def;
    InitOMXParams(&def);
    def.nPortIndex = portIndex;
...
void OMXCodec_cm::initOutputFormat(const sp<MetaData> &inputFormat) {
...
#ifdef OMAP_ENHANCEMENT
int OMXCodec_cm::getNumofOutputBuffers() {
    OMX_PARAM_PORTDEFINITIONTYPE def;
    InitOMXParams(&def);
    def.nPortIndex = kPortIndexOutput;
...
status_t QueryCodecs(
        ...
        Vector<CodecCapabilities_cm> *results) {
    results->clear();
    ...
    for (int index = 0;; ++index) {
        ...
        if (strncmp(componentName, "OMX.", 4)) {
            ...
            results->push();
            CodecCapabilities_cm *caps = &results->editItemAt(results->size() - 1);
        ...
        OMXCodec_cm::setComponentRole(omx, node, !queryDecoders, mime);
...
// static
sp<MediaSource>* createCodec(
        IOMX* omx,
        MetaData* meta, bool createEncoder,
        MediaSource* source,
        const char *matchComponentName,
        uint32_t flags) {

    const sp<IOMX> spOmx = omx;
	const sp<MetaData> spMeta = meta;
	const sp<MediaSource> spSource = source;
	return new sp<MediaSource>(OMXCodec_cm::Create(
		spOmx,
		spMeta,
		createEncoder,
		spSource,
