...
status_t AudioPlayer::start(bool sourceAlreadyStarted) {
    ...
    if (!sourceAlreadyStarted) {
        ...
        if (err != OK) {
        	LOGE("mSource->start() failed!!!");
            return err;
        }
    }

	LOGE("*** Before initAuidoSink() ***");
	err = initAudioSink();
	LOGE("*** After initAudioSink() ***");
	
	if(err != OK) {
		LOGE("initAudioSink() failed!!!");
		return err;
	}
...
size_t AudioPlayer::fillBuffer(void *data, size_t size) {
    ...
    while (size_remaining > 0)
	{
        ...
        if (mInputBuffer == NULL) {
            ...
            //LOGE("mInputBuffer Size: %d", mInputBuffer->range_length());
...
status_t AudioPlayer::initAudioSink() {
    ...
    if (mAudioSink.get() != NULL)
	{
        ...
        err = mAudioSink->open(
                mSampleRate, numChannels, AUDIO_FORMAT_PCM_SUB_16_BIT,
		...
		LOGE("*** Before start audiosink ***");
        mAudioSink->start();
		LOGE("*** After start audiosink ***");
