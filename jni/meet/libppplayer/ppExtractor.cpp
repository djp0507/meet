	...
	while(mStarted)
	{
		if(isSeeking)
		{
			...
			else
			{
			    ...
			    PP_int32 ec = ((PPBoxHandle*)gPlatformInfo->ppbox)->seek(start_time);
		...
		if (ec == ppbox_success && sample.stream_index==mVideoTrackID)
		{
			...
			if(packet != NULL)
			{
				...
				LOGD("constructed nalu length: %d", length);
				if(length > 0 && mVideoBufferGroup)
				{
...
bool SniffPP(const sp<DataSource> &source, String8 *mimeType, float *confidence) 
{
