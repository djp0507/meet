...
void parse_sps(uint8_t * sps,size_t sps_size,uint8_t *aprofile,uint8_t *alevel,uint32_t *num_ref_frames,uint8_t *interlaced)
{
    ...
    LOGD("AVC Level %d",level);

    *aprofile = profile;
    *alevel = level;

    /* read sps id, first exp-golomb encoded value */
    uint32_t spsID = exp_golomb_ue(&bb);
    LOGD("sps id %u",spsID);

    if (profile == 100 || profile == 110 || profile == 122 || profile == 144) {
        /* chroma format idx */
		uint32_t chroma_format_idx = exp_golomb_ue(&bb);
	    LOGD("chroma format idx %u", chroma_format_idx);
        if (chroma_format_idx== 3) {
            skip_bits(&bb, 1);
        }
        /* bit depth luma minus8 */
        uint32_t bit_depth_luma_minus8 = exp_golomb_ue(&bb);
	    LOGD("bit depth luma minus8 %u", bit_depth_luma_minus8);
        /* bit depth chroma minus8 */
        uint32_t bit_depth_chroma_minus8 = exp_golomb_ue(&bb);
	    LOGD("bit depth chroma minus8 %u", bit_depth_chroma_minus8);
    ...
    uint32_t log2_max_frame_num_minus4 = exp_golomb_ue(&bb);
	LOGD("log2 max frame num minus4 %u", log2_max_frame_num_minus4);
    /* pic_order_cnt_type */
    ...
    *num_ref_frames = exp_golomb_ue(&bb);
    LOGD("AVC No of Ref frames %d",*num_ref_frames);

    /* gaps_in_frame_num_value_allowed_flag */
    skip_bits(&bb, 1);
    /* pic_width_in_mbs */
    width_in_mbs = exp_golomb_ue(&bb) + 1;
    LOGD("pic_width_in_mbs %d",width_in_mbs);
    /* pic_height_in_map_units */
    height_in_map_units = exp_golomb_ue(&bb) + 1;
    LOGD("height_in_map_units %d",height_in_map_units);
    /* frame_mbs_only_flag */
    frame_mbs_only_flag = get_bit(&bb);
    LOGD("AVC frame_mbs_only_flag %d", frame_mbs_only_flag);
