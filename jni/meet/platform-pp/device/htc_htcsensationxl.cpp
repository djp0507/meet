...
//#define LOG_NDEBUG 0
#define LOG_TAG "HTC_HTCSensationXL"
...
class HTC_HTCSensationXL_IOMXRenderer : public IPPOMXRenderer
{
public:
	HTC_HTCSensationXL_IOMXRenderer(sp<IOMXRenderer>& target) 
		: mTarget(target)
	{
	}
	...
	virtual ~HTC_HTCSensationXL_IOMXRenderer()
	{
...
void HTC_HTCSensationXL::FlushCommands()
{
	IPCThreadState::self()->flushCommands();
}

HTC_HTCSensationXL::HTC_HTCSensationXL()
{
...
IPPOMXRenderer* HTC_HTCSensationXL::CreateOMXRenderer(
            //const sp<IOMX> &omx,
...
{	
	...
	return new HTC_HTCSensationXL_IOMXRenderer(target);
}

// static
sp<MediaSource> HTC_HTCSensationXL::CreateOMXCodec(
        //const sp<IOMX> &omx,
...
status_t HTC_HTCSensationXL::getOutputSamplingRate(int* samplingRate, int streamType)
{
	return AudioSystem::getOutputSamplingRate(samplingRate, streamType);
}

status_t HTC_HTCSensationXL::getOutputFrameCount(int* frameCount, int streamType)
{
	return AudioSystem::getOutputFrameCount(frameCount, streamType);
}

bool HTC_HTCSensationXL::canAudioHWDecode()
{
    return true;
}

bool HTC_HTCSensationXL::canAudioHWSBR()
{
    return true;
}
IPPAudioTrack* HTC_HTCSensationXL::createAudioTrack(
        int streamType,
        uint32_t sampleRate,
        int format,
        int channels,
