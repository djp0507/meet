...
//#define LOG_NDEBUG 0
#define LOG_TAG "HTC_HTCIncredibleS"
...
class HTC_HTCIncredibleS_IOMXRenderer : public IPPOMXRenderer
{
public:
	HTC_HTCIncredibleS_IOMXRenderer(sp<IOMXRenderer>& target) 
		: mTarget(target)
	{
	}
	...
	virtual ~HTC_HTCIncredibleS_IOMXRenderer()
	{
...
void HTC_HTCIncredibleS::FlushCommands()
{
	IPCThreadState::self()->flushCommands();
}

HTC_HTCIncredibleS::HTC_HTCIncredibleS()
{
...
IPPOMXRenderer* HTC_HTCIncredibleS::CreateOMXRenderer(
            //const sp<IOMX> &omx,
...
{	
	...
	return new HTC_HTCIncredibleS_IOMXRenderer(target);
}

// static
sp<MediaSource> HTC_HTCIncredibleS::CreateOMXCodec(
        //const sp<IOMX> &omx,
...
status_t HTC_HTCIncredibleS::getOutputSamplingRate(int* samplingRate, int streamType)
{
	return AudioSystem::getOutputSamplingRate(samplingRate, streamType);
}

status_t HTC_HTCIncredibleS::getOutputFrameCount(int* frameCount, int streamType)
{
	return AudioSystem::getOutputFrameCount(frameCount, streamType);
}

bool HTC_HTCIncredibleS::canAudioHWDecode()
{
    return true;
}

bool HTC_HTCIncredibleS::canAudioHWSBR()
{
    return true;
}
IPPAudioTrack* HTC_HTCIncredibleS::createAudioTrack(
        int streamType,
        uint32_t sampleRate,
        int format,
        int channels,
